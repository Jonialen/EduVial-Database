...
-- Función mínima: llámala al hacer LOGIN exitoso
...
DECLARE
  ...
  yesterday  DATE := (now() AT TIME ZONE 'America/Guatemala')::date - INTERVAL '1 day';
  r RECORD;
BEGIN
  -- Toma/crea la fila del usuario
  SELECT * INTO r
  FROM user_streak
  ...
  IF NOT FOUND THEN
    ...
    RETURNING current_streak, longest_streak INTO r.current_streak, r.longest_streak;

    RETURN QUERY SELECT r.current_streak, r.longest_streak;
    RETURN;
  END IF;

  -- Idempotente: ya contó hoy
  IF r.last_activity_date = today THEN
    RETURN QUERY SELECT r.current_streak, r.longest_streak;
    RETURN;
  END IF;

  -- Ayer → hoy: continúa racha
  ...
  -- Se rompió: reinicia a 1 (récord queda igual)
  UPDATE user_streak
